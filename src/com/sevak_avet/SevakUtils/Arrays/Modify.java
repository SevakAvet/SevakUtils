...
public class Modify {
	...
	public static int[][] shift(int[][] source, Shift shift, int howMuch) {
		if (howMuch < 0) {
			throw new IllegalArgumentException("Shift size must be positive!");
		} else if (howMuch == 0) {
			return source;
		}
		...
		case LEFT:
			for (int i = 0; i < source.length; ++i) {
				System.arraycopy(source[i], howMuch, newArr[i], 0,
						source[i].length - howMuch);
				System.arraycopy(source[i], 0, newArr[i], source[i].length
						- howMuch, howMuch);
			}
		...
		case RIGHT:
			for (int i = 0; i < source.length; ++i) {
				System.arraycopy(source[i], source[i].length - howMuch,
						newArr[i], 0, howMuch);
				System.arraycopy(source[i], 0, newArr[i], howMuch,
						source[i].length - howMuch);
			}
		...
		case UP:
			for (int i = 0; i < howMuch; ++i) {
				newArr[source.length - howMuch + i] = Arrays.copyOf(source[i],
						source[i].length);
			}

			for (int i = howMuch; i < source.length; ++i) {
				newArr[i - howMuch] = Arrays
						.copyOf(source[i], source[i].length);
			}
		...
		case DOWN:
			...
			for (int i = source.length - howMuch; i < source.length; ++i) {
				newArr[i - source.length + howMuch] = Arrays.copyOf(source[i],
						source[i].length);
			}
